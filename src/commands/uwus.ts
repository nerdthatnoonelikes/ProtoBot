...
export function run(client: Client, message: Message, args: string[], log: (mode: 'i' | 'w' | 'e', message: string) => void): void {
    ...
    (async () => {
        ...
        msg += '===== UWU LEADERBOARD =====\n';
        msg += '\n';
        msg += ` 1 :: ${u1?.tag ?? '(none)'}${t10[0] ? ` with ${t10[0][0]} uwus` : ''}\n`;
        msg += ` 2 :: ${u2?.tag ?? '(none)'}${t10[1] ? ` with ${t10[1][0]} uwus` : ''}\n`;
        msg += ` 3 :: ${u3?.tag ?? '(none)'}${t10[2] ? ` with ${t10[2][0]} uwus` : ''}\n`;
        msg += ` 4 :: ${u4?.tag ?? '(none)'}${t10[3] ? ` with ${t10[3][0]} uwus` : ''}\n`;
        msg += ` 5 :: ${u5?.tag ?? '(none)'}${t10[4] ? ` with ${t10[4][0]} uwus` : ''}\n`;
        msg += ` 6 :: ${u6?.tag ?? '(none)'}${t10[5] ? ` with ${t10[5][0]} uwus` : ''}\n`;
        msg += ` 7 :: ${u7?.tag ?? '(none)'}${t10[6] ? ` with ${t10[6][0]} uwus` : ''}\n`;
        msg += ` 8 :: ${u8?.tag ?? '(none)'}${t10[7] ? ` with ${t10[7][0]} uwus` : ''}\n`;
        ...
        if (!uintop) {
            ...
            if (u) {
                ...
                msg += `${pos < 10 ? ` ${pos}` : pos} :: ${usr.tag} with ${tops[pos - 1][0]} uwus`;
            }
        }
        msg += '```';
        message.reply(msg);
